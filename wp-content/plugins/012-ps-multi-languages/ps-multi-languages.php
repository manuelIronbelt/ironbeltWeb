	...
	function bread_crumb( $type = 'list', $echo = false ) {
		...
		if ( $type == 'string' ) {
			...
			foreach ( $bread_crumb_arr as $ancestor ) {
	            ...
	            } elseif( isset($ancestor['no_link']) && $ancestor['no_link'] === true ){
	                $output .= '    <li class="' . implode( ' ', $classes ) . '">' . apply_filters( 'the_title', $ancestor['title'] ) . '</li>' . "\n";
	            } else {
                        $target_txabi = ( isset($ancestor['target']) )? $ancestor['target'] : "";
	                $output .= '    <li class="' . implode( ' ', $classes ) . '"><a href="' . $ancestor['link'] . '" '. $target_txabi .'>' . apply_filters( 'the_title', $ancestor['title'] ) . '</a></li>' . "\n";
	            }    
	            $cnt++;
			}
			$output .= '</ul>' . "\n";
