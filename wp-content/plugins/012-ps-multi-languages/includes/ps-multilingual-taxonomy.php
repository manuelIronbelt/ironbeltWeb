	...
	*/
	...
	function Start(){
		...
		if ( is_admin( ) ){
			...
			add_action( $taxonomy_txabi . '_add_form_fields'  		, array( &$this, 'admin_custom_taxonomy_add' ) );
			add_action( $taxonomy_txabi . '_edit_form_fields'  		, array( &$this, 'admin_custom_taxonomy_edit' ) );
			add_action( $taxonomy_txabi . '_pre_add_form'				, array( &$this, 'taxonomy_explain_text' ) );
			...
			//表示する項目（item）
			add_filter( 'manage_edit-' . $taxonomy_txabi . '_columns'	, array( &$this, 'custom_taxonomy_columns' ) );
			add_filter( 'manage_'. $taxonomy_txabi .'_custom_column'	, array( &$this, 'display_custom_taxonomy_column' ), 10, 3 );
